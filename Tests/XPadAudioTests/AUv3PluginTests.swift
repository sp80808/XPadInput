...
public enum AUv3PluginTestSuite {
    public static func runTests(
        ...
        do {
            ...
            var rightBuffer = [Float](repeating: 0, count: Int(frameCount))
            ...
            leftBuffer.withUnsafeMutableBufferPointer { leftPtr in
                rightBuffer.withUnsafeMutableBufferPointer { rightPtr in
                    var abl = AudioBufferList(
                        mNumberBuffers: 2,
                        mBuffers: (
                            AudioBuffer(mNumberChannels: 1, mDataByteSize: frameCount * 4, mData: leftPtr.baseAddress),
                            AudioBuffer(mNumberChannels: 1, mDataByteSize: frameCount * 4, mData: rightPtr.baseAddress)
                        )
                    )

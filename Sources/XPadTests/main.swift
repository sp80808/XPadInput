...
func runAllTests() {
    ...
    suite("XPadCore: PitchClass, Interval, Note, Scale, Chord") {
        ...
        test("Chord Voicings, Inversions & Styles") {
            ...
            var drop2 = Chord(root: .c, quality: .major7, voicingStyle: .drop2)
            assertEqual(drop2.voicedNotes(baseOctave: 3).count, 4)

            var acoustic = Chord(root: .c, quality: .major, voicingStyle: .guitarAcoustic)
            assertEqual(acoustic.voicedNotes(baseOctave: 3).count, 5)

            var shell = Chord(root: .c, quality: .major7, voicingStyle: .shellJazz)
